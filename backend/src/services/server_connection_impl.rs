...
impl ServerConnection {
    ...
    pub fn get_users(&self) -> Result<Vec<User>> {
        let users = self.db.find(Table::Users, vec![Filter::Users(UsersFilter::All)], None)?;
        let u = users
            .into_iter()
            .map(|x| {
    ...
    pub fn register_user(&mut self, user: User) -> Result<()> {
        ...
        let all_users = self.get_users()?;
        if all_users.iter().any(|u| u.email.to_lowercase() == user.email.to_lowercase()) {
    ...
    pub fn login(&mut self, email: String, password: String) -> Result<()> {
        let all_users = self.get_users()?;
        let user = all_users.iter().find(|u| u.email.to_lowercase() == email.to_lowercase()).ok_or_else(|| anyhow!("User not found."))?; // if none, user not found
            ...
            match session.role.to_lowercase().as_str() {
                "student" => {
                    ...
                    if student.is_empty() {
                        ...
                        let default_account = StudentAccount {
                            id: 0,
                            student_id: session.id,
                            advisor_id: 0,
                            discipline: "General".to_string(),
                            enrollment: "Active".to_string(),
                            cgpa: 0.0,
                            can_grad: false,
                            cur_credit: 0,
                            cum_credit: 0,
    ...
    pub fn generate_statistics(&self) -> Result<Statistics> {
        let all_users = self.get_users()?;
        let registered_users = all_users.len() as i32;
        let suspended_users = all_users.iter().filter(|u| u.suspended).count() as i32;
        let faculty_members = all_users.iter().filter(|u| u.role.to_lowercase() == "teacher").count() as i32;
        let active_students = all_users.iter().filter(|u| u.role.to_lowercase() == "student").count() as i32 - suspended_users;
        
        // Get graduated students from student accounts table - use All filter and filter in memory
        let all_student_accounts = self.db.find(
            Table::StudentAccount, 
            vec![Filter::StudentAccount(StudentAccountFilter::All)], 
            None
        )?;
        let graduated_students = all_student_accounts
            .into_iter()
            .filter_map(|x| {
                if let ReceiverType::StudentAccount(account) = x {
                    Some(account)
                } else {
                    None
                }
            })
            .filter(|account| account.can_grad)
            .count() as i32;
        
        // Get all courses and departments using the correct filter approach
        let all_courses = self.db.find(Table::Courses, vec![Filter::Courses(CoursesFilter::All)], None)?;
        let courses = all_courses.len() as i32;
        
        let all_departments = self.db.find(Table::Departments, vec![Filter::Departments(DepartmentsFilter::All)], None)?;
        let departments = all_departments.len() as i32;
...
impl ServerConnection {
    ...
    fn update_user_as_student(&mut self, mut user: User) -> Result<()> {
        let all_users = self.get_users()?;
        let u = all_users.iter().find(|u| u.id == user.id).ok_or_else(|| anyhow!("User not found."))?;
