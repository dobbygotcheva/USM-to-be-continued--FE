...
use crate::models::table_models::{User, TeacherAccount, Courses};
use crate::services::server_connection_impl::*;
...
pub async fn get_students() -> impl Responder {
    let conn = ServerConnection::new();
    let students = conn.get_users();
    match students {
        Ok(s) => {
            let students = s.into_iter().filter(|u| u.role.to_lowercase() == "student").collect::<Vec<_>>();
            let json = serde_json::to_string(&students);
            match json {
                Ok(j) => HttpResponse::Ok().body(j),
                Err(e) => HttpResponse::InternalServerError().json(json!({"error": e.to_string()})),
            }
...
pub async fn get_teachers() -> impl Responder {
    let conn = ServerConnection::new();
    let teachers = conn.get_users();
    match teachers {
        Ok(t) => {
            let teachers = t.into_iter().filter(|u| u.role.to_lowercase() == "teacher").collect::<Vec<_>>();
            let json = serde_json::to_string(&teachers);
            match json {
                Ok(j) => HttpResponse::Ok().body(j),
                Err(e) => HttpResponse::InternalServerError().json(json!({"error": e.to_string()})),
            }
...
pub async fn new_department(req: HttpRequest) -> impl Responder {
    ...
    login_macro!(login_email, login_password, conn);
    if !conn.is_admin() {
...
pub async fn get_courses() -> impl Responder {
    ...
    let users = match conn.get_users() {
        Ok(t) => {
            let teachers = t.into_iter().filter(|u| u.role.to_lowercase() == "teacher").collect::<Vec<_>>();
            let json = serde_json::to_string(&teachers);
            match json {
...
pub async fn get_self(req: HttpRequest) -> impl Responder {
    ...
    login_macro!(email, password, conn);
...
#[get("/self")]
pub async fn self_alias(req: HttpRequest) -> impl Responder {
    // Copy the body of get_self here, since get_self may not be callable as a function
    let mut conn = ServerConnection::new();
    let request_headers = req.headers();
